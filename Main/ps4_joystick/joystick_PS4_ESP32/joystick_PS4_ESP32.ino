...
// Urutan 16 Bit
    /***************************************************************************************************************
    * UP * DOWN * LEFT * RIGHT * R1 * L1 * PSButton * OPTIONS * R3 * L3 * R2 * L2 * SQUARE * CIRCLE * TRIANGLE * CROSS *
    ****************************************************************************************************************
...
void setup() {
  ...
  //Serial2.begin(9600);
  PS4.attach(choice);
  PS4.begin("08:b6:1f:28:e7:32");   // Black Stick
  //PS4.begin("b0:05:94:b9:d2:b0"); // Blue Stick
...
void loop() {
  ...
  if (millis() - prevTime >= 25) {
    ...
    if (!DEBUG) {
      Serial2.write(MSB);
      Serial2.write(LSB);
      Serial2.write(RightY);
      Serial2.write(LeftY);
      Serial2.write(RightX);
      Serial2.write(LeftX);
    }
  }
