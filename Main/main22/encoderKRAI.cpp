...
//Copy Constructor
encoderKRAI::encoderKRAI(const encoderKRAI &enc) : channelA_(enc.pinA), channelB_(enc.pinB)
{
    encoding_ = enc.encoding_;;
                         ...
                         Encoding encoding) : channelA_(channelA), channelB_(channelB)
{
    ...
    //and on channel B.
    channelA_.rise(callback(this, &encoderKRAI::encode));
    channelA_.fall(callback(this, &encoderKRAI::encode));

    //If we're using X4 encoding, then attach interrupts to channel B too.
    if (encoding == X4_ENCODING) {
        channelB_.rise(callback(this, &encoderKRAI::encode));
        channelB_.fall(callback(this, &encoderKRAI::encode));
    }
}
...
long encoderKRAI::getPulsesTotal(void) {
