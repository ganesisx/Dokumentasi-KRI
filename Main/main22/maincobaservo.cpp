...
encoderKRAI encMotor[]={
    ...
    encoderKRAI(PA_6, PA_7, 330),        // Motor 1
...
float base = 0;
int analog_RightY=127, analog_LeftY=127; // inisialisasi analog joystick sumbu Y
int analog_RightX=127, analog_LeftX=127; // inisialisasi analog joystick sumbu Y
...
float vxMode = 70, vyMode = 70, betaMode = 10; //Setpoint vx, vy, dan kecepatan putar
...
float Jepit_Lepastable[] = {790, 650}; //jepit, lepas
...
int main(){
    ...
    servo.setAngleLimit(0, ADDRESS_CCW_ANGLE_LIMIT, 1);
    ...
    servo.setSpeed(900, 2);
    servo.setSpeed(900, 3);
    servo.setSpeed(100, 1);
...
void taskComputeSpeed(){
 ...
 void decideCase(){
    ...
    if ((controlSig & 32768) == 32768){ //Up
        ...
        vy = sqrt(pow(analog_LeftY, 2) + pow(analog_LeftX,2)) * sin(theta_joystick + PI/4);
    }
    ...
    if ((controlSig & 8192) == 8192){ //Left
        ...
        vx = sqrt(pow(analog_LeftY, 2) + pow(analog_LeftX,2)) * sin(theta_joystick + PI/4);
    }
    ...
    if ((controlSig & 2048) == 2048){ //R1
        ...
        beta = betaMode * (analog_RightX - 127) / 128;
    }
    ...
    if ((controlSig & 8) == 8){ //Square
        if ((us_ticker_read()/1000)-spdChangeFlag >= 250){
            spdChangeFlag = us_ticker_read()/1000;
            if(!up){
                servo.move_to(Naik_Turuntable[0], 10);
                up = true;
            }
            else if (!circle){
    ...
    // This is how to change the servo ID
    //servo.setID(13, 2);
    //printf("%x\n", servo.getPosition(3));
}
